...
client.on('message', msg => {
  const userId = msg.author.id,
        ...
        scrimFormatPattern = /(((mon|fri|sun)(day)?|tue(sday)?|wed(nesday)?|thu(rsday)?|sat(urday)?)+ ((0?[1-9]|1[0-2])-(0?[1-9]|1[0-9]|2[0-9]|3[0-1]))+ ((1[0-2]|0?[1-9])(:)?([0-5][0-9]) ([AaPp]?[Mm]?)?)+ .*((inferno)|(nuke)|(mirage)|(dust2)|(train)|(overpass)|(cache)|\/))/gims
  ...
  if(!isBot) {
    ...
    if (msg.channel.name == "role-assigner") {
       if (msg.content.indexOf("play.esea.net/users/") > -1) {
        ...
        } else if(msg.content.toLowerCase().indexOf("nice job") > -1 || msg.content.toLowerCase().indexOf("thanks") > -1 || msg.content.toLowerCase().indexOf("thank you") > -1) {
